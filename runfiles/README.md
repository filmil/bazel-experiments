...
For some reason I could not find one.

The scenario under test is as follows:
* There is a script in repo @runfiles, called `@runfiles//:binary` which
uses runfiles to call a library `//:library`.
...
```
This part was copied verbatim from [bazel documentation][bd], which is surprisingly
difficult to find.
...
[dd]: https://bazel.googlesource.com/bazel/+/refs/heads/release-7.0.0-pre.20230302.1rc1/tools/bash/runfiles/runfiles.bash
...
teh call to `$(runfiles_current_repository)` evaluates to `_main`. But if called from
